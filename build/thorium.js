...
class THORUS{
  ...
  th = function(elementHTML , elementRef , ui){
    ...
    var th_proto = {
      ...
      active : Var(false),
      //
      "_css" : Var(("prop" in elementRef && "style" in elementRef.prop ? (function(ref){
        let cssArray = ref.prop.style.toString().split(' ').join('').split(';');
      ...
      //
      activeStyle : Var(("prop" in elementRef && "activeStyle" in elementRef.prop ? (function(ref){
        elementHTML.removeAttribute("activeStyle");
        let cssArray = ref.prop.activeStyle.toString().split(' ').join('').split(';');
        return Array.from({length : cssArray.length} , function(x,i){
          let cssField = cssArray[i].split(':');
          if(cssField.length == 2)return { attr : cssField[0] , value : cssField[1] }
        }).filter((x,i) => x).reduce((acc,cur) => ({...acc , [cur.attr]:cur.value}) , {})
      ...
      //
      hoverStyle : Var(("prop" in elementRef && "hoverStyle" in elementRef.prop ? (function(ref){
        elementHTML.removeAttribute("hoverStyle");
        let cssArray = ref.prop.hoverStyle.toString().split(' ').join('').split(';');
        return Array.from({length : cssArray.length} , function(x,i){
          let cssField = cssArray[i].split(':');
          if(cssField.length == 2)return { attr : cssField[0] , value : cssField[1] }
        }).filter((x,i) => x).reduce((acc,cur) => ({...acc , [cur.attr]:cur.value}) , {})
      ...
      updateNodeID : function(){
        var parentnodeid;
        try{
          parentnodeid = this.e.parentNode.nodeID.Value;
          this.e.nodeID.Value = parentnodeid++;
        }catch(err){
          // parentnodeid n'est pas définis
          this.e.nodeID.Value = 1;
        }
      },
      ...
      updatePosition : function(){
        ...
        this.e.position.Value = position;
      ...
      turnActive : function() {
        var element = this.e;
        if(element.active.Value == false){
          element.classList.add('active');
          element.active.Value = true;

          // appel du style au changement d'état actif de l'élément onActive
          if(Object.keys(self.hoverStyle.Value).length > 0)self.Style()["@onActive"]();
          ...
          element.active.Value = false;

          // appel du style au changement d'état actif de l'élément onUnactive
          if(Object.keys(self.hoverStyle.Value).length > 0)self.Style()["@onUnactive"]();
      ...
      Style : function(arg = null){
        ...
        function style(cssArg,_self){
          ...
          this.__proto__.Reset = function(){
            if("_css" in this.element.th)this.element.th.Style(this.element.th._css.Value);
          }

          this.__proto__._define = function(arg){
            if(typeof arg == "object" && !Array.isArray(arg))this.element.th._css.Value = arg;
          }

          this.__proto__["@onActive"] = function(){
            this.element.th.cssStates().push('active');
            if("activeStyle" in this.element.th)this.element.th.Style().Add(this.element.th.activeStyle.Value)
          }

          this.__proto__["@onUnactive"] = function(){
            this.element.th.cssStates().pop('active');
            for(const e of this.element.th.cssStates().History()){
              if(e == 'default')this.element.th.Style().Reset();
              else this.element.th.Style().Add(this.element.th[`${e}Style`].Value);
            }
          }

          this.__proto__["@onHover"] = function(){
            this.element.th.cssStates().push('hover');
            if("hoverStyle" in this.element.th)this.element.th.Style().Add(this.element.th.hoverStyle.Value)
          }

          this.__proto__["@onUnhover"] = function(){
            this.element.th.cssStates().pop('hover');
            for(const e of this.element.th.cssStates().History()){
              if(e == 'default')this.element.th.Style().Reset();
              else this.element.th.Style().Add(this.element.th[`${e}Style`].Value);
            }
            // if("_css" in this.element.th)this.element.th.Style().Reset();
          }
      ...
      cssStates : function(){
        return new (function(_this){
          ...
          this.__proto__.History = function(){
            return _this._cssStates.Value.history;
          }

          this.__proto__.push = function(arg){
            let states = _this._cssStates.Value;
            states.history.push(arg);
            states.current = states.history[states.history.length - 1];
            states.last = ( states.history.length > 1 ? states.history[states.history.length - 2] : states.history[states.history.length - 1] );
            _this._cssStates.Value = states;
          }

          this.__proto__.pop = function(filter){
            let states = _this._cssStates.Value;
            let rv = states.history.reverse();
            let v = false;
            if(filter)states.history = Array.from({length : states.history.length} , function(x,i){
              if(rv[i] != filter || v == true)return rv[i];
              if(rv[i] == filter)v = true;
            }).filter((x,i) => x).reverse();
            states.current = states.history[states.history.length - 1];
            states.last = ( states.history.length > 1 ? states.history[states.history.length - 2] : states.history[states.history.length - 1] );
            _this._cssStates.Value = states;
          }

          this.__proto__.Current = function(){
            return _this._cssStates.Value.current;
          }

          this.__proto__.Last = function(){
            return _this._cssStates.Value.last;
          }
...
*/
    var th_handlers = {
      ...
      mouseenter : function(e = null){ // fonction d'entrée du curseur sur l'élément
        ...
        if(Object.keys(self.hoverStyle.Value).length > 0)self.Style()["@onHover"]();
      ...
      mouseleave : function(e = null){ // fonction de sortie du curseur sur l'élément
        ...
        if(Object.keys(self.hoverStyle.Value).length > 0)self.Style()["@onUnhover"]();
  ...
  * x = value ; o = options
  */
  Var = function(x = null , o){
    if(!o)o = {};
    if('writable' in o == false)o.writable = true;

    function Var(value , options){
      const _this = this;
      // blockage de l'utilisation de l'objet
      this.__proto__.freeze = function(){
        return Object.freeze(this);
      }

      this.__proto__.Type = function(v){
        ...
        else if(!isNaN(new Number(v))){ // si value est un nombre
      ...
      this.__defineSetter__('Value', function(x){
        let define = (this.Value ? true : false); // A t-il déjà été définis ?
        // si jamais définis mais writable false permission une seule assignation
        if(!define && this.__proto__[`#_writable`] == false)this.__proto__[`#_value`] = x;
        // si il est writable permission d'écriture
        else if (this.__proto__[`#_writable`]){
          // vérification du type
          if(_this.Type(x) == _this.__proto__[`#_dataType`])this.__proto__[`#_value`] = x;
          else if (!define){
            this.__proto__[`#_value`] = x;
            Object.defineProperty(this.__proto__, '#_dataType', {
              value: _this.Type(x),
              writable: false
            });
          }
          else console.error(`Data type does not match. Value is '${_this.Type(x)}' , '${_this.__proto__[`#_dataType`]}' is expected.`);
        }
      ...
      Object.defineProperty(this.__proto__, '#_value', {
        value: value,
      ...
      Array.from({length : Object.keys(options).length} , function(x,i){
          Object.defineProperty(_this.__proto__, `#_${Object.keys(options)[i]}`, {
...
THORIUM_ENGINE.prototype.css = function (arg = null) {
...
class Button extends thorium.components{
  constructor(arg){
    ...
    super({
      type:'button',
      prop : arg.prop,
      childrens : ('text' in arg ? [new Text(arg.text,'center')] : arg.childrens),
      proto : arg.proto
    })
  }
}
...
class List extends thorium.components{
  constructor(type = 'ul',arg){
    super({
      type:(type == 'ul' ? type : 'ol'),
      prop : (typeof arg == 'object' && 'prop' in arg ? (function(x){
        const prop = x.prop;
        delete x.prop;
        return prop;
      })(arg) : {} ),
      childrens : (function(x){
        console.log(x);
        x = (typeof arg == 'object' && 'childrens' in arg ? (function(x){
          console.log(x.childrens);
          return x.childrens;
        })(arg) : x );
        if(!Array.isArray(x))console.error("Erreur 'childrens' ne correspond pas à type Array<UIelement>");
        return Array.from({length : x.length} , function(y,i){
          return {
            type:'li',
            prop : {},
            childrens : [x[i]]
...
class DropDown extends thorium.components{
  constructor(arg){
    super({
      type : 'dropDown',
      prop : {id : ('id' in arg ? arg.id : '') , style:css({
        height: "2ch",
        width: "50%",
        margin: "auto",
        display: "grid",
        border: "1px solid black",
        "grid-template-rows": "2ch 0",
        "z-index":"2",
        "background-color": "white",
      })},
      childrens : [
        new Input({prop:{id : 'dropResult' , placeholder : ('placeholder' in arg ? arg.placeholder : '' ) , style : css({
          "user-select" : "none",
          "background-color": "transparent",
          border: "none",
        })}}),
        new Container({
          prop : {id : 'DropResContainer' , style : css({
            display: "none",
            background : "red",
            "max-height" : "10ch",
            "overflow-y" : "auto",
      ...
      proto : {
        ...
        push : function(value){
          new UI([new Button({
            prop : {text:value , style : css({background : "transparent"})},
            proto : {
              _value : value,
